...
import { getFirestore, collection, addDoc, getDocs, deleteDoc, doc, query, orderBy, limit, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";

const firebaseConfig = {
    apiKey: "AIzaSyBsmjjvh8aoIKtyGg7cq8RQ6NlUbrNvomU",
    authDomain: "replant-9e831.firebaseapp.com",
    projectId: "replant-9e831",
    storageBucket: "replant-9e831.firebasestorage.app",
    messagingSenderId: "852288400846",
    appId: "1:852288400846:web:0beb0989aac3cae42dcf9f",
    measurementId: "G-Y3XCFGXVZJ"
};
...
async function submitSurplus(e) {
    ...
    try {
        await addDoc(collection(db, 'surplus_entries'), surplus);
...
async function matchNearbyNGO(surplusKg) {
    ...
    const ngos = snapshot.docs.map(d => ({ id: d.id, ...d.data() })).filter(o => o.org_type === 'ngo');
...
async function updateSurplusList() {
    const snapshot = await getDocs(query(collection(db, 'surplus_entries'), orderBy('created_at', 'desc'), limit(10)));
    const surplus = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
...
async function loadOrganizations() {
    const snapshot = await getDocs(query(collection(db, 'organizations'), orderBy('created_at', 'desc')));
    const orgs = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
...
async function updateDashboard() {
    ...
    const surplusSnap = await getDocs(collection(db, 'surplus_entries'));

    const orgs = orgsSnap.docs.map(d => d.data());
    const surplus = surplusSnap.docs.map(d => d.data());
...
async function updateImpactReport() {
    const snapshot = await getDocs(collection(db, 'surplus_entries'));
    const surplus = snapshot.docs.map(d => d.data());
